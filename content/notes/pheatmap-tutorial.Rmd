...
```{r non-uniform-density, fig.height=3}
...
```{r uniform-color-breaks}
mat_breaks <- seq(min(mat), max(mat), length.out = 10)


```
```{r uniform-color-breaks-detail, fig.height=2, echo=FALSE}
...
```{r uniform-color-breaks-bars, fig.height=3, echo=FALSE}
...
```
```{r quantile-color-breaks-detail, fig.height=2, echo=FALSE}
dat_colors <- data.frame(
  xmin = mat_breaks[1:(length(mat_breaks)-1)],
  xmax = mat_breaks[2:length(mat_breaks)],
  ymin = 0,
...
```
```{r quantile-color-breaks-bars, fig.height=3, echo=FALSE}
dat2 <- as.data.frame(table(cut(
  mat, mat_breaks
)))
dat2$fill <- inferno(nrow(dat2))
...
```{r pheatmap-quantile-dendsort-example}
...
```{r pheatmap-column-labels, fig.width = 18}
